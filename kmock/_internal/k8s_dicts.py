...
from kmock._internal import dicts
...
class ObjectVersion(MutableMapping[str, Any]):
    ...
    def __ge__(self, other: Mapping[str, Any]) -> bool:
        match other:
            case collections.abc.Mapping():
                return dicts.match_dict(self, other, strict=False)
            case _:
    ...
    def __le__(self, other: Mapping[str, Any]) -> bool:
        match other:
            case collections.abc.Mapping():
                return dicts.match_dict(other, self, strict=False)  # we are the pattern!
            case _:
                return NotImplemented
    ...
    def __eq__(self, other: object) -> bool:
        match other:
            case collections.abc.Mapping():
                return dicts.match_dict(self, other, strict=True)
            case _:
                return NotImplemented
    ...
    * ``<=`` & ``>=`` recursively compare against a partial pattern.
    ...
    def patch(self, changes: Mapping[str, Any] | None = None, /, **kwargs: Any) -> None:
        ...
        if self.deleted:
            ...
            new_version = dicts.patch_dict(old_version, changes or {}, **kwargs)
